...
class BlueAvailabilityRemoveAgent(BaseAgent):
    ...
    def get_action(self, observation, action_space):
        ...
        if self.last_action is not None and self.last_action == 'Monitor':
            for host_name, host_info in [(value['System info']['Hostname'], value) for key, value in observation.items() if key != 'success']:
                if host_name not in self.host_list and host_name != 'User0' and 'Processes' in host_info and len([i for i in host_info['Processes'] if 'PID' in i]) > 1:
...
class BlueAvailabilityRestoreAgent(BaseAgent):
    def __init__(self, agent_name=None):
        self.host_list = []
        self.last_action = None
        self.agent_name = agent_name
    ...
    def get_action(self, observation, action_space):
        ...
        if self.last_action is not None and self.last_action == 'Monitor':
            for host_name, host_info in [(value['System info']['Hostname'], value) for key, value in observation.items() if key != 'success']:
                if host_name not in self.host_list and host_name != 'User0' and 'Processes' in host_info and len([i for i in host_info['Processes'] if 'Process Name' in i]) > 0:
                    # only DoS.exe gets added
                    self.host_list.append(host_name)
        # assume a single session in the action space
        session = list(action_space['session'].keys())[0]
        if len(self.host_list) == 0:
...
class BlueAvailabilityMisinformAgent(BaseAgent):
    def __init__(self, agent_name=None):
        self.host_list = []
        self.last_action = None
        self.agent_name = agent_name
