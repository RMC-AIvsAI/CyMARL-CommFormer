...
from CybORG.Simulator.Actions import PrivilegeEscalate, ExploitRemoteService, DiscoverRemoteSystems, DiscoverNetworkServices, Tamper, Sleep


class RedIntegrityAgent(BaseAgent):
    ...
    def get_action(self, observation, action_space):
        ...
        for hostname in hostnames:
            ...
            if hostname in self.host_ip_map and self.host_ip_map[hostname] not in self.exploited_ips:
                continue
            if hostname in self.tampered_hosts:
                continue
            self.tampered_hosts.append(hostname)
            self.last_host = hostname
            self.last_action = "Tamper"
            return Tamper(hostname=hostname, agent='Red', session=session)
