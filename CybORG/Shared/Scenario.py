...
from CybORG.Shared.BlueRewardCalculator import (
    AvailabilityRewardCalculator, ConfidentialityRewardCalculator,
    HybridAvailabilityConfidentialityRewardCalculator,
    IntegrityRewardCalculator, OTRewardCalculator)
from CybORG.Shared.RedRewardCalculator import (DisruptRewardCalculator,
                                               DoSRewardCalculator,
                                               HybridImpactPwnRewardCalculator,
                                               PwnRewardCalculator,
                                               TamperRewardCalculator)
...
class ScenarioAgent(CybORGLogger):
    ...
    def load(cls, agent_name: str, agent_info: dict):
        return cls(agent_name=agent_name,
                   ...
                   internal_only=agent_info.get('internal_only'),
        )
...
class ScenarioHost:
    def __init__(self, hostname, processes=None, host_type='host', system_info=None, user_info=None, interface_info=None, services=None, image=None, info=None, aws_info=None,
                 confidentiality_value: str = None, integrity_value: str = None, availability_value: str = None, respond_to_ping: bool = True, starting_position=np.array([0.0, 0.0])):
        self.hostname = hostname
    ...
    def get_integrity_value(self, default):
        return self.integrity_value if self.integrity_value is not None else default
...
class Scenario(CybORGLogger):
    ...
    def _get_reward_calculator(self, team_name, reward_calculator, adversary):
        ...
        if reward_calculator == "Baseline":
            ...
            calc = DisruptRewardCalculator(team_name, self)
        ...
        elif reward_calculator == 'BlueConfidentiality':
            calc = ConfidentialityRewardCalculator(team_name, self, adversary)
        elif reward_calculator == 'RedConfidentiality':
            calc = PwnRewardCalculator(team_name, self)
        elif reward_calculator == 'BlueAvailability':
            calc = AvailabilityRewardCalculator(team_name, self, adversary)
        elif reward_calculator == 'RedAvailability':
            calc = DoSRewardCalculator(team_name, self)
        elif reward_calculator == 'BlueIntegrity':
            calc = IntegrityRewardCalculator(team_name, self, adversary)
        elif reward_calculator == 'RedIntegrity':
            calc = TamperRewardCalculator(team_name, self)
        else:
            raise ValueError(f"Invalid calculator selection: {reward_calculator} for agent {team_name}")
        return calc
    ...
    def get_teams(self) -> list:
        return list(self.team_agents.values())
