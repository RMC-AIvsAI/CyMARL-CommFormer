...
class AgentInterface:
    ...
    def create_reward_calculator(self, reward_calculator: str, team_name: str, scenario: Scenario) -> RewardCalculator:
        calc = None
        if reward_calculator == "Baseline":
            calc = BaselineRewardCalculator(team_name)
        elif reward_calculator == 'Pwn':
            calc = PwnRewardCalculator(team_name, scenario)
        elif reward_calculator == 'Disrupt':
            calc = DisruptRewardCalculator(team_name, scenario)
            ...
            calc = HybridAvailabilityConfidentialityRewardCalculator(team_name, scenario)
        elif reward_calculator == 'HybridImpactPwn':
            calc = HybridImpactPwnRewardCalculator(team_name, scenario)
         # scenario specific calculators
        elif reward_calculator == 'BlueConfidentiality':
            calc = ConfidentialityRewardCalculator(team_name, scenario)
        elif reward_calculator == 'RedConfidentiality':
            calc = PwnRewardCalculator(team_name, scenario)
        elif reward_calculator == 'BlueAvailability':
            calc = AvailabilityRewardCalculator(team_name, scenario)
        elif reward_calculator == 'RedAvailability':
            calc = DoSRewardCalculator(team_name, scenario)
        elif reward_calculator == 'BlueIntegrity':
            calc = IntegrityRewardCalculator(team_name, scenario)
        elif reward_calculator == 'RedIntegrity':
            calc = TamperRewardCalculator(team_name, scenario)
        else:
